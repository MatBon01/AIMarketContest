...
class TrainerAgentAdapter(Agent):
    ...
    @staticmethod
    def convert_if_trainer(trained_entity: Union[Agent, Trainer], agents_in_simulation: list[str]) -> Agent:
    ...
    def policy(self, last_round_all_agents_prices: list[Price], identity_index: int) -> Price:
        observations = {agent_name: last_round_all_agents_prices for agent_name in self.agents_in_simulation}
